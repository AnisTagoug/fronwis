...
const ListDepratments = () => {
  ...
  useEffect(() => {
    fetchEmployeeData(); // Call the function to fetch data when the component mounts
  }, []);

  const fetchEmployeeData = async () => {
    try {
      // Fetch departments data
      const departmentssResponse = await axios.get("http://localhost:3001/departments/all");
      const departmentss = departmentssResponse.data;

      // Fetch employees data
      const employeesResponse = await axios.get("http://localhost:8093/employee/all");
      const employees = employeesResponse.data;

      // Process the data and create the 'departmentsData' array
      const departmentsData = departmentss.map((department) => {
        const employeestoInject = employees.filter((employee) => employee.department === department.title);
        return {
